...
public:
	...
	struct compare_node_by_potential
	{
		bool operator()(const ECBSNode* n1, const ECBSNode* n2) const
		{
			if (n1->potential == n2->potential)
			{
				if (n1->distance_to_go == n2->distance_to_go)
				{
					if (n1->sum_of_costs + n1->cost_to_go == n2->sum_of_costs + n2->cost_to_go)
			...
			return n1->potential <= n2->potential;
		}
	};
	...
	pairing_heap< ECBSNode*, compare<ECBSNode::compare_node_by_potential> >::handle_type potential_handle;

	ECBSNode* parent;
	list< pair< int, pair<Path, int> > > paths; // new paths <agent id, <path, min f>>	
	AdjEdges ma_lb;  // <meta_agents, fmin>
